...
public class StatsCollectorCoda implements StatsCollector, MetricGetters {
    ...
    {
        final StatsCollector.Gauge zeroGauge = new Gauge() {
            @Override
            public long value() {
                return 0;
            }
    ...
    public StatsCollectorCoda(final ClusterId clusterId, final StatsCollectorFactoryCoda.MetricNamingStrategy namer) {
        this.namer = namer;
    ...
    public MetricName createName(final String metric) {
        return namer.createName(clusterId, metric);
    }
    ...
    //
    // -------------------------------------------------------------------------
    // Methods for enabling testing of the MP container only. Serveral Unit tests
    // read stats, which in the prior MBean implementation
    ...
    public void stop() {
        Metrics.shutdown();
        for (final String name : METRIC_NAMES) {
    ...
    private static class CodaTimerContext implements StatsCollector.TimerContext {
        private final com.yammer.metrics.core.TimerContext ctx;

        private CodaTimerContext(final com.yammer.metrics.core.TimerContext ctx) {
            this.ctx = ctx;
        }
    ...
    public StatsCollector.TimerContext outputInvokeStarted() {
        return new CodaTimerContext(outputInvokeDuration.time()) {
            ...
            public void stop() {
                super.stop();
                lastOutputCycleMillis.set(System.currentTimeMillis() - startTimeMillis);
            }
        };
    }
    ...
    @Override
    public synchronized void setMessagesPendingGauge(final StatsCollector.Gauge currentMessagesPendingGauge) {
        if (currentMessagesPendingGauge != null)
            this.currentMessagesPendingGauge = currentMessagesPendingGauge;
    }

    @Override
    public synchronized void setMessagesOutPendingGauge(final StatsCollector.Gauge currentMessagesOutPendingGauge) {
