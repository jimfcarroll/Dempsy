...
import net.dempsy.util.SocketTimeout;
...
public class TcpSender implements Sender {
    ...
    protected TcpSender(final TcpDestination destination, final StatsCollector statsCollector,
            final Histogram batching,
            final long maxNumberOfQueuedOutgoing, final long socketWriteTimeoutMillis,
            final long batchOutgoingMessagesDelayMillis, final int mtu) throws MessageTransportException {
    ...
    public long getBatchOutgoingMessagesDelayMillis() {
        return batchOutgoingMessagesDelayMillis;
    }
    ...
    public void setMaxNumberOfQueuedMessages(final long maxNumberOfQueuedMessages) {
        this.maxNumberOfQueuedMessages = maxNumberOfQueuedMessages;
    }
    ...
    public final Histogram getBatchingHistogram() {
        return batching;
    }
    ...
    private final long calcDelay(final long nextTimeToSend) {
    ...
    // of the messages.
    private final void incrementMessageNotSent(final StatsCollector stats, final byte[] messageBytes, final long batchCount,
            final boolean batchOutgoingMessages) {
        if (stats != null) {
            ...
            public void run() {
                ...
                try {
                    ...
                    while (senderKeepRunning.get()) {
                        try {
                            messageBytes = batchOutgoingMessages ? sendingQueue.poll(calcDelay(nextTimeToSend), TimeUnit.MILLISECONDS)
                                    : sendingQueue.take();
            ...
            private DataOutputStream getDataOutputStream() throws MessageTransportException, IOException {
                if (dataOutputStream == null) // socket must also be null.
                {
                    ...
                    if (isLocalAddress == IsLocalAddress.Unknown) {
                        if (socket.isBound()) {
                            final InetAddress localSocketAddress = socket.getLocalAddress();
                            isLocalAddress = (Arrays.equals(localSocketAddress.getAddress(), destination.inetAddress.getAddress()))
    ...
    protected void stop() {
        final Thread t = senderThread.get();
     ...
     */
    protected Socket makeSocket(final TcpDestination destination) throws IOException {
