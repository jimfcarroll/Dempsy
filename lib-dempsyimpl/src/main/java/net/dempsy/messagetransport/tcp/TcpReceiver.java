...
public class TcpReceiver implements Receiver {
    ...
    protected TcpReceiver(final DempsyExecutor executor, final boolean failFast) {
    ...
    public synchronized void start() throws MessageTransportException {
        ...
        if (!failFast && overflowHandler != null)
            logger.warn(
                    "TcpReceiver/TcpTransport is configured with an OverflowHandler that will never be used because it's also configured to NOT 'fail fast' so it will always block waiting for messages to be processed.");
                    ...
                    try {
                        ...
                        synchronized (clientThreads) {
                            ...
                            if (!stopMe.get()) {
                                ...
                                final ClientThread clientThread = makeNewClientThread(clientSocket);
                                final Thread thread = new Thread(clientThread, "Client Handler for " + getClientDescription(clientSocket));
        ...
        if (executor == null) {
            final DefaultDempsyExecutor defexecutor = new DefaultDempsyExecutor();
    ...
    */
    ...
    public synchronized void stop() {
        ...
        try {
            if (messageTransportListener != null)
                messageTransportListener.shuttingDown();
        } catch (final Throwable th) {
        ...
        synchronized (clientThreads) {
            for (final ClientThread ct : clientThreads)
                ct.stop();
        ...
        synchronized (eventLock) {
            if (!eventSignaled) {
                try {
                    eventLock.wait(500);
                } catch (final InterruptedException e) {} // wait for 1/2 second
            }
    ...
    @Override
    public void setListener(final Listener messageTransportListener) {
        this.messageTransportListener = messageTransportListener;
    }

    @Override
    public synchronized void setStatsCollector(final StatsCollector statsCollector) {
    ...
    public TcpDestination doGetDestination() throws MessageTransportException {
        try {
            final InetAddress inetAddress = useLocalhost ? InetAddress.getLocalHost() : TcpTransport.getFirstNonLocalhostInetAddress();
            if (inetAddress == null)
    ...
    */
    public void setUseEphemeralPort(final boolean useEphemeralPort) {
        this.port = -1;
    }
    ...
    */
    ...
    protected void bind() throws MessageTransportException {
        if (serverSocket == null || !serverSocket.isBound()) {
            try {
                final InetSocketAddress inetSocketAddress = new InetSocketAddress(destination.inetAddress,
                        destination.port < 0 ? 0 : destination.port);
    ...
    protected class ClientThread implements Runnable {
        ...
        protected ClientThread(final Socket clientSocket) throws IOException {
                ...
                while (!stopMe.get() && !stopClient.get()) {
                    try {
                                ...
                                try {
                                    ...
                                    executor.submitLimited(new DempsyExecutor.Rejectable<Object>() {
                                        ...
                                        public Object call() throws Exception {
                                            final boolean messageSuccess = messageTransportListener.onMessage(message, failFast);
                                    ...
                                    try {
                                        messageAsString = (messageBytes == null ? "null" : messageBytes.toString());
                                    } catch (final Throwable th) {
                                        messageAsString = "(failed to convert message to a string)";
                                    }
                        ...
                        if (clientIsApparentlyGone != null) {
                            final String logmessage = "Client " + getClientDescription(clientSocket)
                                    + " has apparently disconnected from sending messages to " + destination;
                            if (logger.isDebugEnabled())
                        ...
                        logger.error(MarkerFactory.getMarker("FATAL"),
                                "Completely unexpected error. This problem should be addressed because we should never make it here in the code.",
                                th);
    ...
    private static String getClientDescription(final Socket socket) {
        try {
