 ...
 * 
 * <p>
 * Notice, when using a dependency injection framework, you have the ability to configure your prototypes ({@link Mp}s) and {@link Adaptor}s independently from Dempsy. The Dempsy {@link ApplicationDefinition}
 * would then be the point where you define the topology of processing chain. This nicely decouples the two concerns.
 * </p>
 * 
 * <p>
...
public class ApplicationDefinition {
    ...
    public ApplicationDefinition(final String applicationName, final ClusterDefinition... clusterDefinitions) throws IllegalArgumentException {
        this.applicationName = applicationName;
        add(clusterDefinitions);
    }
     ...
     */
    public ApplicationDefinition setClusterDefinitions(final List<ClusterDefinition> clusterDefs) throws IllegalArgumentException {
        this.clusterDefinitions.clear();
        clusterDefs.stream().forEach(this::append);
        return this;
    }
    ...
    /**
     * Get the {@link ClusterDefinition} that corresponds to the given cluster name within this ApplicationDefinition.
     */
    public ClusterDefinition getClusterDefinition(final String clusterName) {
        return getClusterDefinition(new ClusterId(applicationName, clusterName));
    }
     ...
     */
    public ApplicationDefinition add(final ClusterDefinition... clusterDefs) throws IllegalArgumentException {
        Arrays.stream(clusterDefs).forEach(this::append);
        return this;
    }
    ...
    public void validate() throws IllegalStateException {
        ...
        if (clusterDefinitions == null || clusterDefinitions.size() == 0)
            throw new IllegalStateException("The application \"" + SafeString.valueOf(applicationName) + "\" doesn't have any clusters defined.");

        final Set<ClusterId> clusterNames = new HashSet<ClusterId>();

        for (final ClusterDefinition clusterDef : clusterDefinitions) {
            if (clusterDef == null)
                throw new IllegalStateException("The application definition for \"" + applicationName + "\" has a null ClusterDefinition.");

            if (clusterNames.contains(clusterDef.getClusterId()))
                throw new IllegalStateException(
                        "The application definition for \"" + applicationName + "\" has two cluster definitions with the name \""
                                + clusterDef.getClusterId().clusterName + "\"");
    ...
    private void append(final ClusterDefinition clusterDef) throws IllegalArgumentException {
        if (clusterDef == null)
            throw new IllegalArgumentException("The application definition for \"" + applicationName + "\" has a null ClusterDefinition.");
        clusterDef.setParentApplicationDefinition(this);
        clusterDefinitions.add(clusterDef);
    }
